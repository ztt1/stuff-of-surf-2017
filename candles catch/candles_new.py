...
import numpy as np
'''
this file works based on Oanda platform to download instruments' data that you need, 
you should install v20 engine first. 
Details check:https://github.com/oanda/v20-python-samples
'''
def main():
	"""
	...
	for i in range(0,18):
		...
		# Fetch the candles
		#
		response = api.instrument.candles(instruments, **kwargs)
		...
		#	printer.print_candle(candle)
			list_c.append(candle.mid.c)
			list_time.append(candle.time)
			list_h.append(candle.mid.h)
			list_l.append(candle.mid.l)
			list_o.append(candle.mid.o)
			list_v.append(candle.volume)
		
		datas = pd.date_range(date, freq=frequent1,periods=counts+1)
		date=datas[-1]
		
	
	output=pd.DataFrame({'Date':list_time,
						 'Close':list_c,
						 'High':list_h,
						 'Low':list_l,
						 'Open':list_o,
	...
	output.to_csv(instruments+'.csv',index=False,sep=',')
